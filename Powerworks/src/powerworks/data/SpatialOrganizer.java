...
public class SpatialOrganizer<T extends Collidable> {
	...
	objects.stream().filter((T t) -> intersects(x, y, width, height, t.getXPixel() + t.getHitbox().xStart, t.getYPixel() + t.getHitbox().yStart, t.getHitbox().width, t.getHitbox().height)).forEach(returnObj::add);
	return returnObj;
    }
    
    public boolean anyIntersecting(int x, int y, int width, int height) {
	for(T t : objects)
	    if(intersects(x, y, width, height, t.getXPixel() + t.getHitbox().xStart, t.getYPixel() + t.getHitbox().yStart, t.getHitbox().width, t.getHitbox().height))
		return true;
	return false;
    }
	...
	List<T> returnObj = new ArrayList<T>();
	objects.stream().filter((T t) -> contains(x, y, width, height, t.getXPixel() + t.getHitbox().xStart, t.getYPixel() + t.getHitbox().yStart, t.getHitbox().width, t.getHitbox().height)).forEach(returnObj::add);
	return returnObj;
    }
